...
def send_push_message(reg_id, title, msg):
    ...
    if isinstance(reg_id, list):
        if len(reg_id) > 0:
            return push_service.notify_multiple_devices(registration_ids=reg_id, message_title=title, message_body=msg, sound='Default')
    else:
        return push_service.notify_single_device(registration_id=reg_id, message_title=title, message_body=msg, sound='Default')
